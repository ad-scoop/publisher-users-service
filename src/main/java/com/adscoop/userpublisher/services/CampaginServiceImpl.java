...
public class CampaginServiceImpl implements CampaginService {
    ...
    public Optional<Campagin> findCampaginsByUserTokenAndName(String campaginname, String token) throws Exception {
        try{
            return Optional.ofNullable(session.queryForObject(Campagin.class, "match (u)-[:CAMPAGIN_HAS_USER]->(c) where u.token = '"+token +"' and c.campagin_name='"+campaginname+"' return c", Collections.emptyMap()));
        }catch (Exception e){
            throw new Exception(e);
        }
    }
